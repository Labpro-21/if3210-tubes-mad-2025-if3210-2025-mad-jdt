...
class UserRepository(private val tokenManager: TokenManager, private val userService: UserService) {
    ...
    suspend fun editProfile(location: String?, profilePhoto: File?): Result<ProfileResponse> {
        ...
        return try {
            ...
            val profilePhotoBody =
                    profilePhoto?.let {
                        val requestFile = it.asRequestBody("image/*".toMediaTypeOrNull())
                        MultipartBody.Part.createFormData("profilePhoto", it.name, requestFile)
                    }

            val response = userService.editProfile("Bearer $token", locationBody, profilePhotoBody)
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                if (profileResponse.isSuccessful && profileResponse.body() != null) {
                    ...
                    Result.failure(
                            Exception(
                                    "Failed to fetch profile after edit: ${profileResponse.message()}"
                            )
                    )
                }
            } else {
